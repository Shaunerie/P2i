...
@njit
def allongement_normal_grain_paroigauche(indice_temps, grain, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite):
    ...
    distance_a_la_gauche = abs(-Agauche * POSITION[indice_temps, grain, 0] + 1 * POSITION[indice_temps, grain, 1] - Cgauche) / np.sqrt(Agauche**2 + 1)
    penetration_gauche = distance_a_la_gauche - RAYON[grain]

    return penetration_gauche

@njit
def allongement_normal_grain_paroidroite(indice_temps, grain, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite):
    # Distance à la paroi, droite d'équation: A*x + B*y + C = 0. Ici B=1, A=-Agauche/droite et C=-Cgauche/droite.
    # La distance es alors donné par la relation : d = abs(A*x + B*y + C) / sqrt(A**2 + B**2)
    distance_a_la_droite = abs(-Adroite * POSITION[indice_temps, grain, 0] + 1 * POSITION[indice_temps, grain, 1] - Cdroite) / np.sqrt(Adroite**2 + 1)
    penetration_droite = distance_a_la_droite - RAYON[grain]

    return penetration_droite

@njit
def allongement_tangentiel_grain_paroigauche(indice_temps, grain, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite, vecteur_orthogonal_paroi_gauche):
    k = 0
    position_i = POSITION[indice_temps-k, grain]
    while allongement_normal_grain_paroigauche(indice_temps, grain, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite) < 0 and k < indice_temps:
        k += 1
        position_i = POSITION[indice_temps-k, grain]
...
@njit
def allongement_tangentiel_grain_paroidroite(indice_temps, grain, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite, vecteur_orthogonal_paroi_droite):
    k = 0
    position_i = POSITION[indice_temps-k, grain]
    while allongement_normal_grain_paroigauche(indice_temps, grain, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite) < 0 and k < indice_temps:
        k += 1
    ...
    for grain1 in range(nb_grains):
        if mise_a_jour[grain1]:
            ...
            if POSITION[indice_temps, grain1, 1] - RAYON[grain1] >= debut_du_trou:
                penetration_droite = allongement_normal_grain_paroidroite(indice_temps, grain1, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite)
                penetration_gauche = allongement_normal_grain_paroigauche(indice_temps, grain1, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite)

                if penetration_gauche < 0:
                    force_resultante += -raideur_normale * penetration_gauche * vecteur_orthogonal_paroi_gauche
                    #force tangentielle:
                    #allongement_tangentiel = allongement_tangentiel_grain_paroigauche(indice_temps, grain1, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite, vecteur_orthogonal_paroi_gauche)
                    #force_resultante += -raideur_tangentielle * allongement_tangentiel * vecteur_orthogonal_paroi_gauche

                    #amortissement:
                    #derivee_allongement = derivee_allongement_normal_grain_paroigauche(grain1, indice_temps, VITESSE, vecteur_orthogonal_paroi_gauche)
                    #force_resultante += - AMORTISSEMENT[grain1] * derivee_allongement * vecteur_orthogonal_paroi_gauche

                elif penetration_droite < 0:
                    force_resultante += -raideur_normale * penetration_droite * vecteur_orthogonal_paroi_droite
                    print(-raideur_normale * penetration_droite * vecteur_orthogonal_paroi_droite)

                    #force tangentielle:
                    #allongement_tangentiel = allongement_tangentiel_grain_paroidroite(indice_temps, grain1, POSITION, VITESSE, Agauche, Adroite, Cgauche, Cdroite, vecteur_orthogonal_paroi_droite)
                    #force_resultante += -raideur_tangentielle * allongement_tangentiel * vecteur_orthogonal_paroi_droite

                    #amortissement:
                    #derivee_allongement = derivee_allongement_normal_grain_paroidroite(grain1, indice_temps, VITESSE, vecteur_orthogonal_paroi_droite)
                    #force_resultante += - AMORTISSEMENT[grain1] * derivee_allongement * vecteur_orthogonal_paroi_droite
                ...
                if mise_a_jour[grain2]:
                    ...
                    if grain1 != grain2:
                        ...
                        if allongement_normal < 0:
                            ...
                            force_contact += - AMORTISSEMENT[grain1] * derivee_allongement_normal * vecteur_normal
...
temps = 0
indice_temps = 0
pas_de_temps = (1/raideur_normale)*(1/2) #s
